...
def load_csv_traj(file_tracer):
    ...
    for j in range(len(file_tracer)):
        df_csv = pd.read_csv(file_tracer[j])
        list_df[j] = df_csv
    return list_df

def load_csv_traj_couple(file_tracer):
    df_csv = pd.read_csv(file_tracer)
    list_df = df_csv
    return list_df
...
def load_xml_traj(file_tracer):
    ...
    for j in range(len(file_tracer)):
        ...
        list_df[j] = pd.DataFrame(data=d_coord_list)
    return list_df
...
def gpytool(calib_px, calib_fr):
    ...
    file_save.close()
    root.destroy()
    df_gpytool = pd.DataFrame(data={'traj_ID': [], 'alpha':[], 'D (um2/s**a)':[]})
    for i in range(len(df_traj)):
        print(str(i+1)+' / '+str(len(df_traj)))
        if df_traj[0].shape[1] == 3:
            traj_exp = np.array([df_traj[i][df_traj[i].keys()[1]].tolist(), df_traj[i][df_traj[i].keys()[2]].tolist()]).T
        else:
            traj_exp = np.array([df_traj[i][df_traj[i].keys()[1]].tolist(), df_traj[i][df_traj[i].keys()[2]].tolist(), df_traj[i][df_traj[i].keys()[3]].tolist()]).T
        vec_t = np.array([df_traj[i][df_traj[i].keys()[0]].tolist()]).T
        res = get_D_alpha(traj_exp-np.mean(traj_exp,axis=0),vec_t)
        res.x[0] = res.x[0]*calib_px**2*calib_fr**(-res.x[1])
        df_gpytool = pd.concat([df_gpytool, pd.DataFrame(data={'traj_ID': [ntpath.basename(file_tracer[i][0:len(file_tracer[i])-4])], 'alpha':[res.x[1]], 'D (um2/s**a)':[res.x[0]]})])
    df_gpytool.to_csv(file_save.name, index=False)
    return df_gpytool

def gpytool_couple(calib_px, calib_fr):
    root = tkinter.Tk(className='Open trajectories', )
    ...
    for i in range(len(list_traj_couple)):
        ...
        if traj1_common.shape[1] == 3:
            traj_exp1 = np.array([traj1_common[traj1_common.keys()[1]].tolist(), traj1_common[traj1_common.keys()[2]].tolist()]).T
            traj_exp1 = traj_exp1-traj_exp1[0,:]
            traj_exp2 = np.array([traj2_common[traj2_common.keys()[1]].tolist(), traj2_common[traj2_common.keys()[2]].tolist()]).T
            traj_exp2 = traj_exp2-traj_exp2[0,:]
        else:
            traj_exp1 = np.array([traj1_common[traj1_common.keys()[1]].tolist(), traj1_common[traj1_common.keys()[2]].tolist(), traj1_common[traj1_common.keys()[3]].tolist()]).T
            traj_exp1 = traj_exp1-traj_exp1[0,:]
            traj_exp2 = np.array([traj2_common[traj2_common.keys()[1]].tolist(), traj2_common[traj2_common.keys()[2]].tolist(), traj2_common[traj2_common.keys()[3]].tolist()]).T
            traj_exp2 = traj_exp2-traj_exp2[0,:]
        vec_t = np.array([traj1_common[traj1_common.keys()[0]].tolist()]).T
        ...
        df_gpytool_couple = {'traj1': [list_traj_couple['traj_1'][i]],'traj2': [list_traj_couple['traj_2'][i]], 'alpha_1':[res.x[1]], 'D1 (um2/s**a)':[res.x[0]],'alpha_2':[res.x[3]], 'D2 (um2/s**a)':[res.x[2]],'alpha_back':[res.x[5]], 'D_back (um2/s**a)':[res.x[4]], 'traj_back_exp':traj_back_exp.T.tolist()}
